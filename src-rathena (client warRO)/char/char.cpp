...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		(p->show_equip != cp->show_equip) || (p->queue_delay != cp->queue_delay)
	)
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`hotkey_rowshift`='%d', `clan_id`='%d', `title_id`='%lu', `show_equip`='%d', `queue_delay_timer`='%d'"
			...
			p->hotkey_rowshift, p->clan_id, p->title_id, p->show_equip, p->queue_delay,
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`unban_time`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`clan_id`,`title_id`,`show_equip`,`queue_delay_timer`"
