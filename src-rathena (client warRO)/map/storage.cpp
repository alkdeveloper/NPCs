...
int storage_additem(struct map_session_data* sd, struct s_storage *stor, struct item *it, int amount)
{
	...
	if( battle_config.stuff_enable ) {
		// BG Stuff
		if( it->card[0]==CARD0_CREATE && MakeDWord(it->card[2],it->card[3])==battle_config.stuff_bg_reserved_id && !(battle_config.stuff_enable_storage&0x01)  )
		{
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return 1;
		}
		...
		// GvG Stuff
		if( it->card[0]==CARD0_CREATE && MakeDWord(it->card[2],it->card[3])==battle_config.stuff_gvg_reserved_id && !(battle_config.stuff_enable_storage&0x02) )
		{
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return 1;
		}
		...
		// PvP Stuff
		if( it->card[0]==CARD0_CREATE && MakeDWord(it->card[2],it->card[3])==battle_config.stuff_pvp_reserved_id && !(battle_config.stuff_enable_storage&0x04) )
		{
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return 1;
		}
...
bool storage_guild_additem(struct map_session_data* sd, struct s_storage* stor, struct item* item_data, int amount)
{
	...
	if( battle_config.stuff_enable ) {
		// BG Stuff
		if( item_data->card[0]==CARD0_CREATE && MakeDWord(item_data->card[2],item_data->card[3])== battle_config.stuff_bg_reserved_id && !(battle_config.stuff_enable_guild_storage&0x01) )
		{
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return false;
		}
		...
		// GvG Stuff
		if( item_data->card[0]==CARD0_CREATE && MakeDWord(item_data->card[2],item_data->card[3])== battle_config.stuff_gvg_reserved_id && !(battle_config.stuff_enable_guild_storage&0x02) )
		{
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return false;
		}
		...
		// PvP Stuff
		if( item_data->card[0]==CARD0_CREATE && MakeDWord(item_data->card[2],item_data->card[3])== battle_config.stuff_pvp_reserved_id && !(battle_config.stuff_enable_guild_storage&0x04) )
		{
			clif_displaymessage(sd->fd, msg_txt(sd,264));
			return false;
		}
...
bool storage_guild_additem2(struct s_storage* stor, struct item* item, int amount) {
	...
	if( battle_config.stuff_enable ) {
		// BG Stuff
		if (item->card[0] == CARD0_CREATE && MakeDWord(item->card[2], item->card[3]) == battle_config.stuff_bg_reserved_id && !(battle_config.stuff_enable_guild_storage&0x01))
			return false;

		// GvG Stuff
		if (item->card[0] == CARD0_CREATE && MakeDWord(item->card[2], item->card[3]) == battle_config.stuff_gvg_reserved_id && !(battle_config.stuff_enable_guild_storage&0x02))
			return false;

		// PvP Stuff
		if (item->card[0] == CARD0_CREATE && MakeDWord(item->card[2], item->card[3]) == battle_config.stuff_pvp_reserved_id && !(battle_config.stuff_enable_guild_storage&0x04))
			return false;
	}
