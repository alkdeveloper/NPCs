...
int status_calc_mob_(struct mob_data* md, enum e_status_calc_opt opt)
{
	...
	if(flag&4) { // Strengthen Guardians - custom value +10% / lv
		...
		// [CreativeSD]: Battleground Warfare
		if (battle_config.bgw_enable && mapdata->flag[MF_BATTLEGROUND] && md->mob_id == MOBID_EMPERIUM) {
#ifdef RENEWAL
			status->max_hp += 50 * (battle_config.bgw_castle_defense / 5);
#else
			status->max_hp += 1000 * battle_config.bgw_castle_defense;
#endif
			status->hp = status->max_hp;
			status->def += (battle_config.bgw_castle_defense+2)/3;
			status->mdef += (battle_config.bgw_castle_defense+2)/3;
		}
		else if (!gc)
