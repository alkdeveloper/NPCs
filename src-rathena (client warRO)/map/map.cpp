...
void map_reqnickdb(struct map_session_data * sd, int charid)
{
	...
	if( battle_config.stuff_enable ) {
		...
		if( charid == battle_config.stuff_bg_reserved_id ) {
			clif_solved_charname(sd->fd, charid, msg_txt(sd,battle_config.stuff_msg_txt));
			return;
		}
	
		// GvG
		if( charid == battle_config.stuff_gvg_reserved_id ) {
			clif_solved_charname(sd->fd, charid, msg_txt(sd,(battle_config.stuff_msg_txt+2)));
			return;
		}
	
		// PvP
		if( charid == battle_config.stuff_pvp_reserved_id ) {
			clif_solved_charname(sd->fd, charid, msg_txt(sd,(battle_config.stuff_msg_txt+1)));
			return;
		}
	}
...
void map_setcell(int16 m, int16 x, int16 y, cell_t cell, bool flag)
{
	...
	switch( cell ) {
		...
		case CELL_NOBATTLEGROUND: mapdata->cell[j].nobattleground = flag;	break;
		default:
...
void do_final(void){
	...
	do_final_status();
	do_final_unit();
	do_final_queue(); // [CreativeSD]: Queue System
...
int do_init(int argc, char *argv[])
{
	...
	do_init_quest();
	do_init_achievement();
	do_init_queue(); // [CreativeSD]: Queue System
	do_init_npc();
	do_init_unit();
