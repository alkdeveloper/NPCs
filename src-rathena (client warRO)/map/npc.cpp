...
void npc_cashshop_buylist_sub(struct map_session_data* sd, unsigned short itemreq, const char *reg, int discount, int n, unsigned short* item_list, int point, struct npc_data* nd) {
...
int npc_cashshop_buylist(struct map_session_data *sd, int points, int count, unsigned short* item_list)
{
	...
	// [CreativeSD]: Shop Extended (Item/Point Shop Buy Item Event)
	if( nd->master_nd ) {
		if( nd->subtype == NPCTYPE_CASHSHOP ) {
			int cost[2] = { 0,0 };
			npc_shop_currency_type(sd, nd, cost, false);
			npc_cashshop_buylist_sub(sd, 0, "#CASHPOINTS", nd->u.shop.discount, count, item_list, vt, nd->master_nd);
			return ERROR_TYPE_NONE;
		}
	...
	switch(nd->subtype) {
		...
		case NPCTYPE_POINTSHOP:
			if (display) {
				...
				// [CreativeSD]: Shop Extended
				if( strlen(nd->u.shop.descpoint) > 0 && nd->u.shop.descpoint[0] != '\0' )
					sprintf(output, msg_txt(sd, 715), nd->u.shop.descpoint); // Point Shop List: '%s'
				else
...
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	if (type != NPCTYPE_SHOP) {
		if (type == NPCTYPE_ITEMSHOP) nd->u.shop.itemshop_nameid = nameid; // Item shop currency
			...
			// [CreativeSD]: Shop Extended
			memset(nd->u.shop.descpoint, '\0', sizeof(nd->u.shop.descpoint));// Point shop Decription
		}
