...
ACMD_FUNC(senhastorage)
{
	if (sd->npc_id || sd->state.vending || sd->state.buyingstore || sd->state.trading)
		return -1;

	npc_event(sd, "senhastorage::OnSenha", 0);
	return 0;
...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_PAYON,       172, 97 }, //  3=Payon
		{ MAP_ALBERTA,     30, 235 }, //  4=Alberta
#ifdef RENEWAL
		{ MAP_IZLUDE,      128, 146 }, //  5=Izlude (Renewal)
#else
		{ MAP_IZLUDE,      128, 141 }, //  5=Izlude
#endif
		{ MAP_ALDEBARAN,   139, 116 }, //  6=Al de Baran
		{ MAP_LUTIE,       147, 134 }, //  7=Lutie
		{ MAP_COMODO,      209, 143 }, //  8=Comodo
		{ MAP_YUNO,        157, 179 }, //  9=Yuno
		...
		{ MAP_BRASILIS,    195, 215 }, // 29=Brasilis
...
ACMD_FUNC(autotrade) {
	...
	if (presence) {
		clif_displaymessage(fd, "Você está com a presença ativada, utilize o comando @bepresence desligar a presença.");
		return -1;
	}
...
ACMD_FUNC(arealoot) {
	...
	if (sd->area_loot.active) {
		...
		clif_displaymessage(fd, "Arealoot is now off.");
		return 0;
	}
	sd->area_loot.active = true;
	clif_displaymessage(fd, "Arealoot is now on.");
	return 0;
}
	...
	for(;;)
	{
		...
		if (pc_isvip(sd) || pc_get_group_id(sd) == 99) {
			if( md->spawn_timer == INVALID_TIMER )
				...
				snprintf(atcmd_output, sizeof(atcmd_output), "%2d - [%s %3d:%3d] %s", number, "Morreu em", md->bl.x, md->bl.y, md->name);
			clif_displaymessage(fd, atcmd_output);
			} else {
				if( md->spawn_timer == INVALID_TIMER )
...
#include "queue_atcmd_func.inc"

// [CreativeSD]: Battleground Warfare
#include "bgw_atcmd_func.inc"

// [CreativeSD]: Restock System
#include "restock_atcmd_func.inc"
...
// [CreativeSD]: Stuff Items
#include "stuff_atcmd_def.inc"

	// [CreativeSD]: Restock System
#include "restock_atcmd_def.inc"

	// [CreativeSD]: Queue System
#include "queue_atcmd_def.inc"

	// [CreativeSD]: Battleground Warfare
#include "bgw_atcmd_def.inc"
	};
