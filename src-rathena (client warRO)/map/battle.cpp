...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	switch( t_bl->type ) { //Checks on target master
		...
		case BL_MOB:
		{
			...
			if( md->guardian_data && md->guardian_data->guild_id && !mapdata_flag_gvg(mapdata) && !(battle_config.bgw_enable && mapdata->flag[MF_BATTLEGROUND]) )
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_PARTY|BCT_ENEMY) )
		{
			...
			if( s_party && s_party == status_get_party_id(t_bl) && !(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP_NOPARTY]) && !(mapdata_flag_gvg(mapdata) && mapdata->flag[MF_GVG_NOPARTY]) && (!mapdata->flag[MF_BATTLEGROUND] || sbg_id == tbg_id) )
				state |= BCT_PARTY;
			else if ( battle_config.bgw_enable && mapdata->flag[MF_BATTLEGROUND] && map_getcell(s_bl->m, s_bl->x, s_bl->y, CELL_CHKNOBATTLEGROUND))
		...
		if( flag&(BCT_GUILD|BCT_ENEMY) )
		{
			...
			if ( battle_config.bgw_enable && mapdata->flag[MF_BATTLEGROUND] && (sbg_id || tbg_id))
			{
				if (sbg_id == tbg_id)
					state |= BCT_GUILD;
				else
...
#include "queue_battle_config_init.inc"
	...
	// [CreativeSD]: Battleground Warfare
#include "bgw_battle_config_init.inc"

	// [CreativeSD]: Restock System
#include "restock_battle_config_init.inc"

	// [CreativeSD]: Force Unequip Items
...
#include "stuff_battle_config_init.inc"
