	...
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		if( sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory_data[i] )
		{
			...
			if( battle_config.stuff_enable ) {
				if( sd->inventory.u.items_inventory[i].card[0]==CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[i].card[2],sd->inventory.u.items_inventory[i].card[3])== battle_config.stuff_bg_reserved_id && !(battle_config.stuff_enable_sell&0x1) )
					continue;
	
				if( sd->inventory.u.items_inventory[i].card[0]==CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[i].card[2],sd->inventory.u.items_inventory[i].card[3])== battle_config.stuff_gvg_reserved_id && !(battle_config.stuff_enable_sell&0x2) )
					continue;
	
				if( sd->inventory.u.items_inventory[i].card[0]==CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[i].card[2],sd->inventory.u.items_inventory[i].card[3])== battle_config.stuff_pvp_reserved_id && !(battle_config.stuff_enable_sell&0x4) )
					continue;
			}
	...
	case BL_MOB:
		{
			...
			// [CreativeSD]: Battleground Warfare
			if (battle_config.bgw_enable && md->bg_id && md->bg_id != sd->bg_id && map_getmapflag(md->bl.m,MF_BATTLEGROUND))
	...
	case BL_PC:
		{
			...
			// [CreativeSD]: Battleground Warfare
			if( battle_config.bgw_enable && battle_config.bgw_name_position && sd->bg_id )
			{
				struct battleground_data *bg = bg_team_search(sd->bg_id);
				if( bg != NULL && bg->g != NULL )
				{
	...
	case BL_MOB:
		{
			...
			// [CreativeSD]: Battleground Warfare
			if (battle_config.bgw_enable && battle_config.bgw_name_position && md->bg_id && (bg = bg_team_search(md->bg_id)) != NULL && md->desc[0])
			{
#if PACKETVER >= 20150513
				WBUFW(buf, 0) = cmd = 0xa30;
#else
			...
			else if( md->guardian_data && md->guardian_data->guild_id )
			{
#if PACKETVER >= 20150513
				WBUFW(buf, 0) = cmd = 0xa30;
#else
...
void clif_parse_LoadEndAck(int fd,struct map_session_data *sd)
{
	...
	/*else if( battle_config.bgw_enable && mapdata->flag[MF_BATTLEGROUND] )
		clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);*/
	else
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if( battle_config.queue_only_towns && sd->queue_id && !queue_check_town(sd->bl.m) )
		queue_leave(sd, QUEUE_L_TOWN);

	// [CreativeSD]: Force the removal of unauthorized items on a particular map
	// Fix: Stuff Items and Costume System, but works for others.
...
void clif_parse_skill_toid( struct map_session_data* sd, uint16 skill_id, uint16 skill_lv, int target_id ){
	...
	if( SKILL_CHK_GUILD(skill_id) ) {
		// [CreativeSD]: Battleground Warfare
		struct battleground_data *bg = NULL;
		if( battle_config.bgw_enable && sd->bg_id && (bg = bg_team_search(sd->bg_id)) != NULL && bg != NULL )
...
void clif_parse_GuildRequestEmblem(int fd,struct map_session_data *sd)
{
	...
	// [CreativeSD]: Battleground Warfare
	if( battle_config.bgw_enable && guild_id >= (SHRT_MAX-1) )
	{
...
{
	if(!map_getmapflag(sd->bl.m, MF_PVP)) pc_revive_item(sd);
  else clif_displaymessage(sd->fd, msg_txt(sd,451));
}
...
void clif_sendbgemblem_area(struct map_session_data *sd)
{
	...
	// [CreativeSD]: Battleground Warfare
	if( battle_config.bgw_enable && !(battle_config.bgw_enable_cross_swords&0x02) )
		return;
...
// [CreativeSD]: Battleground Warfare
#include "bgw_clif_func.inc"

// [CreativeSD]: Fake Guild Emblem
