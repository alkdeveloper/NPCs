 ...
 *------------------------------------------*/
int mob_spawn_bg(const char* mapname, int16 x, int16 y, const char* mobname, int mob_id, const char* event, unsigned int bg_id, const char* info)
{
	struct mob_data *md = nullptr;
	struct spawn_data data;
	int16 m;
	...
	if( battle_config.bgw_enable && !(battle_config.bgw_enable_cross_swords&0x01) )
			...
			if (mvp_sd && battle_config.announcement_and_log_system) {
				...
				if (it->type == IT_CARD && md->get_bosstype() == BOSSTYPE_MVP && battle_config.mvp_card_announce_system) {
					...
					if (battle_config.mvp_card_announce) {
						char message[128];
						sprintf (message, msg_txt(sd,451), mvp_sd->status.name, md->name, mapindex_id2name(mvp_sd->mapindex), it->name, (float)drop_rate/100);
					...
					if (battle_config.dropped_mvp_card_log) {
						if ( SQL_ERROR == Sql_Query(mmysql_handle, "INSERT INTO dropped_mvp_card_log (account_id, char_name, mvp_id, mvp_name, card_id, card_name, drop_map) VALUES ('%d', '%s', '%d', '%s', '%d', '%s', '%s')", mvp_sd->status.account_id, mvp_sd->status.name, md->mob_id, md->db->sprite, it->nameid, it->name,  mapindex_id2name(mvp_sd->mapindex))) // RMT Log System [Bad]
